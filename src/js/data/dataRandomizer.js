...
function getRandomDateAndTime() {
  ...
  const setMinutes = new Date(setHours).setMinutes(
    getRandomIntInclusive(0, 59)
  );
  const setSeconds = new Date(setMinutes).setSeconds(
    getRandomIntInclusive(0, 59)
  );
  const setMiliseconds = new Date(setSeconds).setMilliseconds(
...
function randomDateAfterGivenDate(givenDate) {
  ...
  const randomMillisecondsToAdd =
    Math.floor(Math.random() * maxDifferenceMilliseconds) + 1;
